...
router.post('/gallery', [
  ...
  try {
    ...
    console.log(`   📸 Next photo number: ${nextNumber}`);
...
router.delete('/gallery/:filename', [
  ...
  try {
    ...
    console.log(`✅ Deleted Gallery record: ${record._id}`);
...
router.post('/about-image', [
  ...
  try {
    ...
    if (sectionNumber) {
      ...
      try {
        ...
        try {
          ...
          console.error(`   ⚠️  Cloudinary upload failed for section ${sectionNumber} (non-fatal):`, cloudinaryError.message);
          ...
          if (!content) {
            ...
            await content.save();
            console.log(`   ✅ Content record created (local path only) for section ${sectionNumber}`);
          }
        }
    ...
    // Upload to Cloudinary (non-blocking - if it fails, still return success with local path)
    try {
      console.log('☁️  Uploading about image to Cloudinary...');
      ...
      console.log('✅ About image uploaded to Cloudinary:', cloudinaryResult.url);
      fileInfo.cloudinaryUrl = cloudinaryResult.url;
      ...
      // Log error but don't fail the upload
      console.error('⚠️  Cloudinary upload failed (non-fatal):', cloudinaryError.message);
      console.error('   Image saved locally, but Cloudinary upload failed. Local path will be used.');
      ...
      if (!content) {
        ...
        await content.save();
        console.log('✅ Content record created (local path only)');
      }
    }
